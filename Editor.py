...
class Editor():
    ...
    def __str__(self):
        ...
        if not self.directory_content:
            ...
            saved = self.directory_content[self.file_index][self.file_path:-4]
            self.saved_maps = MESSAGE_MAP_LOAD + saved.capitalize() + CLR_ATTR
        self.choice = [self.saved_maps] + [
            MESSAGE_MAP_CHOICE_RANDOM_SMALL,
            MESSAGE_MAP_CHOICE_RANDOM_BIG,
            MESSAGE_MAP_CHOICE_DEFINE_SIZE,
            MESSAGE_MAP_CHOICE_QUIT]
        result_str = ""
        for i, maze_map in enumerate(self.choice):
            if i == self.selected:
                result_str += BLACK_ON_WHITE + maze_map + WHITE_TEXT + CLR_ATTR
            else:
    ...
    def display(self):
        ...
        print(CLR_ATTR + "\033[{0};0H{1},{2}".format(self.height + 1,
                                                     self.x, self.y))
        ...
        while no_input:
            ...
            if keystroke == CTRL_C:
                ...
                print(WHITE_TEXT + "\033[{0};0H\033[K{1}\n\033[K".format(
                    self.height + 2, MESSAGE_QUIT_EDITING))
                keystroke = self.keyboard_input(1)
                if keystroke == CTRL_C:
                    os.system('clear')
                    exit()
                ...
                no_input = False
                if (
                        maze_map[self.y][self.x] != LETTER_DOOR and
                ...
                no_input = False
                if (
                        maze_map[self.y][self.x] != LETTER_PLAYER[0] and
